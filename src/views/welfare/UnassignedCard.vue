<template>
  <div class="list-container">
    ...
    <!-- 修改价格弹窗 -->
    <el-dialog title="修改价格" :visible.sync="editPriceDialogVisible" width="50%">
      <el-form :model="editPriceForm" label-width="100px" style="width: 90%;text-align: left;">
        <el-form-item label="修改方式">
          <el-radio-group v-model="editPriceForm.flag">
            <el-radio label="1">按ID范围</el-radio>
            <el-radio label="2">按选中ID</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="开始ID" v-if="editPriceForm.flag === '1'">
          <el-input v-model="editPriceForm.start_id" placeholder="请输入开始ID"></el-input>
        </el-form-item>
        <el-form-item label="结束ID" v-if="editPriceForm.flag === '1'">
          <el-input v-model="editPriceForm.end_id" placeholder="请输入结束ID"></el-input>
        </el-form-item>
        <el-form-item label="新价格">
          <el-input v-model="editPriceForm.edprice" placeholder="请输入新价格"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" style="display: block;text-align: center;">
        <el-button @click="editPriceDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmEditPrice">确定</el-button>
      </span>
    </el-dialog>

    <!-- 生成福利卡弹窗 -->
    <el-dialog title="生成福利卡" :visible.sync="addCardDialogVisible" width="500px">
      <el-form :model="addCardForm" label-width="120px" style="width: 90%;text-align: left;">
      ...
      <span slot="footer" style="display: block;text-align: center;">
        ...
        <el-button type="primary" @click="confirmAddCard">确定生成</el-button>
      </span>
    </el-dialog>

    <!-- 批量分配弹窗 -->
    <!-- 批量分配弹窗 - 修改后 -->
    <el-dialog title="批量分配" :visible.sync="assignDialogVisible" width="50%">
      <el-form :model="assignForm" label-width="120px" style="width: 90%;text-align: left;">
        ...
        <!-- 新增：选择福利卡（从kaFlkList接口获取） -->
        <el-form-item label="选择福利卡">
          <el-select v-model="assignForm.gid" placeholder="请选择福利卡" filterable style="width: 100%;">
            <el-option v-for="item in flkOptions" :key="item.id"
              :label="item.company + ' - ' + item.sale + ' (数量:' + (item.geshu || '0') + ')'" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="价格">
          <el-input v-model="assignForm.price" placeholder="请输入价格"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" style="display: block;text-align: center;">
        ...
        <div>
          <el-button type="danger" size="small" v-if="multipleSelection.length > 1"
            @click="delOptions()">批量删除</el-button>
             <el-button type="primary" size="small" @click="openAddCardDialog">生成福利卡</el-button>
          <el-button type="primary" size="small" @click="openEditPriceDialog" style="display: none;">批量修改价格</el-button>
         
          <el-button type="primary" size="small" @click="openAssignDialog">批量分配</el-button>
        </div>
      </div>
      <el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark" style="width: 100%"
        @selection-change="handleSelectionChange" v-if="tableshow" v-loading="loading" stripe
        ...
        <el-table-column label="卡号" min-width="200" align="center">
          <template slot-scope="scope">
            {{ scope.row.num }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="价格" align="center" min-width="100">
          <template slot-scope="scope">
            <el-input v-model="scope.row.price" style="width: 160px;" size="mini"
              @blur="handleEditPrice(scope.row)"></el-input>
          </template>
        </el-table-column>
...
import { kaList, addKa, delKa, kaEditPrice, kaAllEditPrice, kaExport, kaFlkList, kaBatchPid } from '@/api/modules/card'
...
export default {
  ...
  data() {
    return {
      ...
      assignForm: {
        flag: '1',    // 默认按选中ID
        gid: '',      // 福利卡id
        start_id: '', // flag=1时的开始id
        end_id: '',   // flag=1时的结束id
        ids: '',
        price: ''       // flag=2时的选中id
      },
      flkOptions: [], // 福利卡列表选项（从kaFlkList接口获取）
      flkLoading: false, // 福利卡列表加载状态
  ...
  methods: {
    onAssignFlagChange(val) {
      if (val === '2') {
        this.editPriceForm.ids = this.multipleSelection.length > 0
    ...
    // 获取福利卡列表
    async getFlkList() {
      this.flkLoading = true
      try {
        const res = await kaFlkList()
        if (res.code === 200) {
          this.flkOptions = res.data
        }
      } catch (error) {
        this.$message.error('获取福利卡列表失败')
      } finally {
        this.flkLoading = false
      }
    },
    ...
    openAssignDialog() {
      ...
      this.assignForm = {
        flag: hasSelection ? '2' : '1',  // 有选中默认2，无选中默认1
        gid: '',
        start_id: '',
        end_id: '',
        price: '',
    ...
    async confirmAssign() {
      ...
      if (this.assignForm.flag === '1') {
        ...
        if (!this.assignForm.ids) {
          this.$message.warning('请先在表格中勾选要分配的卡片，或切换到"按ID范围"模式')
          return
        }
      }
      ...
      // ✅ 新增：如果有价格，添加到参数中
      if (this.assignForm.price) {
        params.price = this.assignForm.price
      }
      try {
    ...
    async confirmEditPrice() {
      if (!this.editPriceForm.edprice) {
        this.$message.warning('请输入新价格')
        return
      }
      if (this.editPriceForm.flag === '1' && (!this.editPriceForm.start_id || !this.editPriceForm.end_id)) {
        this.$message.warning('请输入开始ID和结束ID')
        return
      }
      try {
        const params = {
          flag: this.editPriceForm.flag,
          edprice: this.editPriceForm.edprice
        }
        if (this.editPriceForm.flag === '1') {
          params.start_id = this.editPriceForm.start_id
          params.end_id = this.editPriceForm.end_id
        } else {
          params.ids = this.editPriceForm.ids
        }
        const res = await kaAllEditPrice(params)
        if (res.code === 200) {
          this.$message.success(res.msg)
    ...
    // 单个修改价格
    async handleEditPrice(row) {
      try {
        const res = await kaEditPrice({
          id: row.id,
          price: row.price
        })
        if (res.code === 200) {
    ...
    // 确认批量分配
    // confirmAssign() {
    //   if (!this.assignForm.category) {
    //     this.$message.warning('请选择分配分类')
    //     return
    //   }
    //   this.$confirm('确定分配 ' + this.multipleSelection.length + ' 张卡吗？', '提示', {
    //     confirmButtonText: '确定',
    //     cancelButtonText: '取消',
    //     type: 'warning'
    //   }).then(() => {
    //     console.log('批量分配:', this.assignForm, '已选卡:', this.multipleSelection)
    //     this.assignDialogVisible = false
    ...
    delOptions() {
      if (this.multipleSelection.length === 0) {
        this.$message.warning('请先选择要删除的卡片')
        return
      }
      this.$confirm('确定删除选中的卡片吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
