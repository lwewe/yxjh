<template>
  <div class="list-containerclass">
    ...
    <el-card class="search-card" v-if="addBox">
      ...
      <el-table ref="multipleTable" :data="tableData" border v-loading="loading"
        ...
        <el-table-column label="操作" min-width="260" fixed="right" align="center">
          <template slot-scope="scope">
            <el-button size="mini" type="warning" @click="addBoxs(scope.row)">关联产品</el-button>
    ...
    <!-- 添加/编辑弹窗 -->
    <el-dialog :title="isEdit ? '修改合卡分类' : '添加合卡分类'" :visible.sync="dialogVisible" width="500px" @close="resetForm">
    ...
    <div v-if="!addBox">
      <el-card class="search-card mb16" v-if="selectedProducts.length > 0">
        <div class="dflex al-center mb20">
          <el-select v-model="currentHeKaId" placeholder="请选择分类" @change="switchHeKa" style="width:200px;">
...
<script>
import {
  classifyList, addClassify, editClassify, delClassify,
  getClassify, pxClassify, fbClassify,
  fenleiProduct, flProductPage, flProductUpdate, flProductDel
...
export default {
  ...
  data() {
    return {
      ...
      categoryForm: { title: '', px: '', sh: 1, add_time: '' },
      rules: {
  ...
  methods: {
    ...
    openAddDialog() {
      this.isEdit = false
      this.editId = ''
      this.categoryForm = { title: '', px: '', sh: 1, add_time: this.getCurrentTime() }
      ...
      this.$refs.formRef.validate(async (valid) => {
        ...
        try {
          const params = {
            ...
            px: String(this.categoryForm.px),
            sh: String(this.categoryForm.sh),
    ...
    resetForm() {
      this.categoryForm = { title: '', px: '', sh: 1, add_time: '' }
    },
    ...
    async addBoxs(row) {
      ...
      try {
        const res = await fenleiProduct({ id: row.id, kid: this.kid })
        if (res.code === 200) {
          const d = res.data
          // 一级分类
          this.fenleiFuList = d.list_fenlei_fu || []
          // 二级分类（首次加载当前分类对应的二级分类）
          this.fenleiZiList = d.list_fenlei || []
          // 所有分类列表（用于切换）
          this.hekaList = d.kace_list || []
          // 已选产品
    ...
    async switchHeKa(id) {
      ...
      this.fenleiZiList = []
      try {
        const res = await fenleiProduct({ id, kid: this.kid })
        if (res.code === 200) {
          const d = res.data
          this.fenleiFuList = d.list_fenlei_fu || []
          this.fenleiZiList = d.list_fenlei || []
    ...
    async handleClass1Change(val) {
      ...
      // 重新请求当前分类的二级分类
      try {
        const res = await fenleiProduct({ id: this.currentCategoryId, kid: this.kid })
        if (res.code === 200) {
          // 根据选择的一级分类过滤二级分类（实际接口返回所有二级分类，这里保持不变）
          this.fenleiZiList = res.data.list_fenlei || []
        }
      } catch (e) {
        this.fenleiZiList = []
      }
    ...
    async loadProducts() {
      ...
      try {
        ...
        const res = await flProductPage(params)
        if (res.code === 200) {
    ...
    async toggleSelect(item) {
      if (item.selected == 1) {
        // 取消选中
        try {
          const res = await flProductDel({ id: this.currentCategoryId, gid: item.id })
          if (res.code === 200) {
        ...
        // 选中
        try {
          const res = await flProductUpdate({ id: this.currentCategoryId, gid: item.id })
          if (res.code === 200) {
