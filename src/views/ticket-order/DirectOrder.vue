<template>
  <div class="list-container-directorder titles">
    <template v-if="isDirectOrder">
        ...
        <el-form :inline="true" :model="searchForm" class="search-form" style="text-align: left;">
          ...
          <el-form-item label="">
            <el-select v-model="searchForm.search_key" placeholder="搜索字段" class="w120">
              ...
              <el-option label="直充订单号" value="zc_order_no"></el-option>
          ...
          <el-form-item label="">
            <el-input v-model="searchForm.search_val" placeholder="关键词" class="w130"></el-input>
          </el-form-item>
      ...
      <el-card class="search-card">
        ...
        <el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark" style="width: 100%"
          ...
          <el-table-column label="直充信息" align="left" min-width="280">
            <template slot-scope="scope">
              <div v-if="scope.row.zhichong">
                <div>名称：{{ scope.row.zhichong.title }}</div>
                <div>编码：{{ scope.row.code }}</div>
          ...
          <el-table-column label="订单信息" align="left" min-width="290">
            <template slot-scope="scope">
              <div>订单号：{{ scope.row.order_no }}</div>
              <div>直充订单号：{{ scope.row.zc_order_no }}</div>
          ...
          <el-table-column label="操作" min-width="220" fixed="right" align="center">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="warning" @click="updateOrder(scope.row)">更新</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
      ...
      <el-dialog title="商户余额" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
        <el-table :data="tableData2" border style="width: 100%">
          <el-table-column prop="ye" label="账户余额" align="center"></el-table-column>
          <el-table-column prop="je" label="账号" width="360" align="center"></el-table-column>
        </el-table>
      </el-dialog>
    </template>
    <router-view />
...
import { zcorderList, delZcorder, gxtlZcorder, zcExportOrder } from '@/api/modules/orders'
...
export default {
  ...
  data() {
    return {
      ...
      tableData2: [{ ye: '5866.03', je: '悦享聚汇（普） (1811014948984631297)' }],
      dialogVisible: false,
      multipleSelection: [],
      tableData: []
    }
...
async exportOrder() {
      this.$confirm('确认导出当前搜索条件下的所有直充订单吗？', '提示', {
    ...
    async doExportOrder() {
      try {
        ...
        const res = await zcExportOrder(params)
        ...
        if (res.code === 200 && res.data) {
          ...
          try {
            ...
            XLSX.utils.book_append_sheet(wb, ws, '直充订单')
  ...
  `
    ...
    async loadData() {
      ...
      try {
        ...
        const res = await zcorderList(params)
        if (res.code === 200) {
    ...
    batchDelete() {
      ...
      this.$confirm(`确认删除选中的 ${this.multipleSelection.length} 个订单吗？`, '提示', {
        ...
        const res = await delZcorder({ ids })
        if (res.code === 200) {
    ...
    deleteOrder(row) {
      this.$confirm('确认删除该订单吗？', '提示', { type: 'warning' }).then(async () => {
        const res = await delZcorder({ ids: row.id })
        if (res.code === 200) {
          this.$message.success('删除成功')
          this.loadData()
        }
    ...
    updateOrder(row) {
      this.$confirm('确认更新该订单吗？', '提示', { type: 'warning' }).then(async () => {
        const res = await gxtlZcorder({ id: row.id })
        if (res.code === 200) {
    ...
    viewOrder(row) {
      this.$router.push({ name: 'DirectOrderdetails', params: { id: row.id } })
    },

    balanceMy() { this.dialogVisible = true },
  ...
  computed: {
    isDirectOrder() {
      return this.$route.name === 'DirectOrder' || this.$route.path === '/ticket-order/direct'
    }
  }
}
</script>
...
<style>
.list-container-directorder .serchForm .el-form-item {
  margin-bottom: 0;
}
</style>

<style lang="scss" scoped>
.list-container-directorder {
  .pagination {
    background: #fff;
    padding: 15px 20px;
    display: flex;
