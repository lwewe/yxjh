<template>
  <div class="list-container-senceorder titles">
    <template v-if="isScenicOrder">
        ...
        <el-form :inline="true" :model="searchForm" class="search-form" style="text-align: left;">
          <el-form-item label="">
            <el-select v-model="searchForm.search_key" placeholder="搜索字段" class="w120">
              ...
              <el-option label="来一票订单号" value="third_orderid"></el-option>
              <el-option label="用户ID" value="uid"></el-option>
              <el-option label="联系人" value="user_name"></el-option>
              <el-option label="联系电话" value="mobile_number"></el-option>
          ...
          <el-form-item label="">
            <el-select v-model="searchForm.state" placeholder="订单状态" class="w120">
              ...
              <el-option label="预处理" value="0"></el-option>
              <el-option label="处理中" value="1"></el-option>
              <el-option label="处理完成" value="2"></el-option>
              <el-option label="拒绝" value="3"></el-option>
              <el-option label="部分退单" value="4"></el-option>
              <el-option label="全部退单" value="5"></el-option>
              <el-option label="推送失败" value="6"></el-option>
              <el-option label="已出票" value="7"></el-option>
              <el-option label="取消中" value="8"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
      ...
      <el-card class="search-card">
        ...
        <el-table ref="multipleTable" :data="tableData" border style="width: 100%" v-loading="loading"
          ...
          <el-table-column label="景区信息" align="left" min-width="300">
            <template slot-scope="scope">
              <div>景区：{{ scope.row.name }}</div>
              <div>票种：{{ scope.row.piaotype }}</div>
              <div>使用日期：{{ scope.row.use_date }}</div>
              <div>票数：{{ scope.row.resource_data ? scope.row.resource_data[0].quantity : '-' }} 张 <span
                  class="fs10">￥</span>{{ scope.row.total_price }}</div>
            </template>
          </el-table-column>
          <el-table-column label="用户信息" align="left" min-width="200">
            <template slot-scope="scope">
              ...
              <div>联系电话：{{ scope.row.mobile_number }}</div>
              <div>扣款价：<span class="fs10">￥</span>{{ scope.row.deductions_price }}</div>
            </template>
          </el-table-column>
          <el-table-column label="订单信息" align="left" min-width="380">
            <template slot-scope="scope">
              <div>订单号：{{ scope.row.order_no }}</div>
              <div>来一票订单号：{{ scope.row.third_orderid || '-' }}</div>
              <div>下单时间：{{ formatTime(scope.row.add_time) }}</div>
            </template>
          </el-table-column>
          <el-table-column label="状态" align="center" min-width="100">
            <template slot-scope="scope">
              <span :style="{ color: getStateColor(scope.row.status) }">{{ scope.row.third_party_status }}</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" min-width="250" fixed="right" align="center">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="warning" @click="cancelOrder(scope.row)">取消订单</el-button>
...
import { jqorderList, delJqorder, gxJqorder, jqExportOrder, qxJqorder } from '@/api/modules/orders'
...
export default {
  ...
  data() {
    return {
      ...
      searchForm: { search_key: 'order_no', search_val: '', px_key: 'add_time', px_val: 'desc', ks_time: '', end_time: '', state: 'status' },
  ...
  methods: {
    ...
    async exportOrder() {
      this.$confirm('确认导出当前搜索条件下的所有景区订单吗？', '提示', {
    ...
    async doExportOrder() {
      try {
        ...
        // 订单状态（state 是必填参数，默认传 'status' 表示全部）
        if (this.searchForm.state) {
          params.state = this.searchForm.state
        }

        const res = await jqExportOrder(params)
        ...
        if (res.code === 200 && res.data) {
          ...
          try {
            ...
            XLSX.utils.book_append_sheet(wb, ws, '景区订单')
  ...
  `
    ...
    async cancelOrder(row) {
      this.$confirm('确认取消该订单吗？', '提示', { type: 'warning' }).then(async () => {
        try {
          const res = await qxJqorder({ id: row.id })
          if (res.code === 200) {
    ...
    async loadData() {
      ...
      try {
        ...
        if (this.searchForm.state && this.searchForm.state !== 'status') params.state = this.searchForm.state
        const res = await jqorderList(params)
        if (res.code === 200) { this.tableData = res.data.data; this.total = res.data.total; this.currentPage = res.data.current_page }
        else { this.$message.error(res.msg || '获取数据失败') }
      } catch (error) { this.$message.error('获取数据失败') }
      finally { this.loading = false }
    },
    handleSearch() { this.currentPage = 1; this.loadData() },
    handleReset() {
      this.searchForm = { search_key: 'order_no', search_val: '', px_key: 'add_time', px_val: 'desc', ks_time: '', end_time: '', state: 'status' }
    ...
    batchDelete() {
      ...
      this.$confirm(`确认删除选中的 ${this.multipleSelection.length} 个订单吗？`, '提示', { type: 'warning' }).then(async () => {
        await delJqorder({ ids }); this.$message.success('删除成功'); this.loadData()
      }).catch(() => { })
    },
    deleteOrder(row) {
      this.$confirm('确认删除该订单吗？', '提示', { type: 'warning' }).then(async () => {
        await delJqorder({ ids: row.id }); this.$message.success('删除成功'); this.loadData()
      }).catch(() => { })
    },
    updateOrder(row) {
      this.$confirm('确认更新该订单吗？', '提示', { type: 'warning' }).then(async () => {
        await gxJqorder({ id: row.id }); this.$message.success('更新成功'); this.loadData()
      }).catch(() => { })
    },

    viewOrder(row) { this.$router.push({ name: 'ScenicOrderdetails', params: { id: row.id } }) },

    getStateColor(s) {
      const map = { '-1': '#409EFF', 0: '#E6A23C', 1: '#E6A23C', 2: '#67C23A', 3: '#F56C6C', 4: '#F56C6C', 5: '#F56C6C', 6: '#F56C6C', 7: '#67C23A', 8: '#E6A23C' }
      return map[s] || '#303133'
    },
    formatTime(t) {
      if (!t) return ''
  ...
  computed: {
    isScenicOrder() { return this.$route.name === 'ScenicOrder' || this.$route.path === '/ticket-order/scenic' }
  }
}
</script>
...
.list-container-senceorder .serchForm .el-form-item {
  margin-bottom: 0;
}
</style>

<style lang="scss" scoped>
.list-container-senceorder {
  .pagination {
    background: #fff;
    padding: 15px 20px;
    display: flex;
