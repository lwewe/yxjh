<template>
  <div class="cardcarous">
    <el-card class="search-card serchForm">
      <el-form :inline="true" :model="searchForm" class="search-form" style="text-align: left;">
        <el-form-item label="">
          <el-select v-model="searchForm.pid" placeholder="请选择栏目" class="w150" clearable>
            <el-option label="首页专享福利大图" :value="1"></el-option>
            <el-option label="首页专享福利小图" :value="2"></el-option>
            <el-option label="影视会员轮播图" :value="3"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="">
          <el-select v-model="searchForm.sh" placeholder="发布状态" class="w100" clearable>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible" width="50%">
      <el-form :model="form" :rules="formRules" ref="formRef" label-width="100px" style="width: 90%;text-align: left;">
        ...
        <el-form-item label="图片：" prop="img">
          ...
          <div class="upload-tip">建议尺寸：750*300px，大小不超过2MB</div>
        </el-form-item>
    ...
    <el-card class="search-card">
      ...
      <el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark" style="width: 100%"
        ...
        <el-table-column label="图片" width="160" align="center">
          <template slot-scope="scope">
            <el-image :src="scope.row.img" style="width: 100px; height: 60px;" fit="contain"
        ...
        <el-table-column label="标题" min-width="200" align="left">
          <template slot-scope="scope">
            ...
            <span class="blueCl" v-if="scope.row.pid === 1">[首页专享福利大图] </span>
            <span class="blueCl" v-if="scope.row.pid === 2">[首页专享福利小图] </span>
            <span class="blueCl" v-if="scope.row.pid === 3">[影视会员轮播图] </span>
...
import { navList, editPx, editFb, delNav, addNav, getNav, editNav } from '@/api/modules/zhichong'
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        pid: [{ required: true, message: '请选择栏目', trigger: 'change' }],
  ...
  methods: {
    ...
    async handleEdit(row) {
      ...
      try {
        // 调用详情接口获取完整数据
        const res = await getNav({ id: String(row.id) })
        if (res.code === 200) {
          const data = res.data
          this.form = {
            id: data.id,
    ...
    async customUpload(options) {
      ...
      formData.append('file_url', 'kaquan/banner')
    ...
    async updateStatus(row, newVal) {
      ...
      if (this.statusChanging) return  // 防重复提交
      this.statusChanging = true

      try {
        const res = await editFb({ id: row.id })
        if (res.code === 200) {
